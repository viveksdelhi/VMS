...
import { API, token, StreamAPI, userId } from "serverConnection";
...
const Recording = () => {
    ...
    const fetchCameras = async () => {
      try {
        const response = await axios.get(
          `${API}/api/Camera/?user_id=${userId}&pageNumber=1&pageSize=1000`,
          {
  ...
  const fetchRecordings = async () => {
    if (selectedCamera) {
      try {
        const response = await axios.get(
          `${StreamAPI}/list_recordings/${selectedCamera.id}`
        );
  ...
  return (
    <div>
                        ...
                        <a href={`${VideoAPI}/${recording.recordPath}`} >
                          ...
                          >
                            <Box
                              ...
                              onClick={() =>
                                handleShowVideoModal(
                                  `${StreamAPI}/playback/${selectedCamera.id}/${recording}`
                                )
                              }
                            >
                              <VideoLibraryTwoToneIcon sx={{ fontSize: 100 }} />{" "}
                            </Box>
                            ...
                            {/* <Typography variant="body2" sx={{ mt: 1 }}>
                              ...
                              {recording.regDate
                                ...
                                .replace(/-/g, "-")}
                              <br />
                              {new Date(recording.regDate).toLocaleString(
                                "en-US",
                                {
                                  hour: "2-digit",
                                  minute: "2-digit",
                          ...
                          >
                            <IconButton
                              ...
                              //   handleDelete(
                              //     `${StreamAPI}/delete_recording/${selectedCamera.name}/${recording}`
                              //   )
                              // }
                              sx={{ color: "red" }}
                            >
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to delete {selectedrecording}?
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            onClick={() => {
              handleDelete(
                `${StreamAPI}/delete_recording/${selectedCamera.id}/${selectedrecording}`
              );
