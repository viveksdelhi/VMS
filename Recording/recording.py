...
def call_api_periodically_for_camera(camera_id, credit_id=1, event_id=2):
    ...
    while camera_recording_state.get(camera_id, False) and not stop_event.is_set():
        try:
            ...
            recording_api= os.getenv("RECORDING_API")
            recording_api= f"{recording_api}/transaction_update"
            response = requests.post(recording_api, json=data)
            # response = requests.post("https://14.195.152.244:7007/transaction_update", json=data)
