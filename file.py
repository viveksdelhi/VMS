...
BaseUrl = 'http://mpexcisevms.ecosmartdc.com'
MEDIA_FOLDER = os.path.join(os.getcwd(), "media")
if not os.path.exists(MEDIA_FOLDER):
    os.makedirs(MEDIA_FOLDER)

def push_detection_data_to_base_url(self, object_count, object_detect, framePath, alert_type):
        api_url = f'{BaseUrl}:6004/api/CameraAlert/'
...
def callback(ch, method, properties, body):
    try:
        ...
        if MEDIA_FOLDER and camera_ip:
            ...
            filename = date_time.strftime("%Y-%m-%d_%H-%M-%S") + ".jpg"
...
try:
    ...
    channel.basic_consume(queue='image_info', on_message_callback=callback, auto_ack=True)
