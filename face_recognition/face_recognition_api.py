...
def update_camera_details():
    ...
    for camera in cameras:
        ...
        camera_url = camera.get("url", None)
        camera_ip = camera.get("camera_ip", None)
        running = camera.get("running", False)
        user_id = camera.get("user_id",None)
        credit_id = camera.get("credit_id", None)
        
        # Connect to RabbitMQ
        queue_name='camera_details_for_face_recognition'
