...
import { API, LiveFeedUrl, userId } from "serverConnection";
...
const VideoGrid = () => {
  ...
  const fetchDatas = async () => {
    try {
      ...
      const response = await axios.get(`${API}/api/Camera/?user_id=${userId}&page=1&pageSize=100`);
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API}/api/Camera/?user_id=${userId}&page=1&pageSize=100`
        );
        const data = response.data.results;
        const updatedUrls = data.map(
          (camera) => `${LiveFeedUrl}/${camera.id}/stream.m3u8`
        );
  ...
  useEffect(() => {
    ...
    const savedGridSize = Cookies.get("gridSize");
    if (savedGridSize) {
      setGridSize(parseInt(savedGridSize, 10)); // Ensure it's a number
    }
  ...
  const handleSelect = (eventKey) => {
    const newSize = Number(eventKey); // Convert eventKey to a number
    setGridSize(newSize); // Update state
    Cookies.set("gridSize", newSize); // Save to cookies
  };
  ...
  const openFullScreen = async () => {
    if (divRef.current && document.body.contains(divRef.current)) {
      ...
      console.error("Fullscreen element is not connected to the DOM.");
    }
  };

  const Container = styled.div`
    background-color:#F8F3EC; /* Bootstrap dark color */
    position: relative;
    max-width: 100%;
  `;

  const InnerContainer = styled.div`
    background-color:#F8F3EC; /* Bootstrap warning color */
  `;
...
const VideoPlayer = ({ url, camera, style, allcameradata }) => {
  ...
  return (
    ...
    >
      ...
      {videoStatus === "loading" && (
        <div
          style={{position: "absolute",top: "50%",left: "50%",transform: "translate(-50%, -50%)",color: "white",fontSize: "20px",zIndex: 10,
          }}
        >
      ...
      {/* Camera ID and Name */}
      <div
        style={{color: "red",  display: "inline",  position: "absolute",  bottom: "10px",  left: "10px",}}
      >
        {camera.id}
      </div>
      ...
      {videoStatus === "valid" && (
        <>
          <div
            style={{position: "absolute",top: "2px",right: "10px",display: "flex",
            }}
          >
          ...
          <div
            style={{position: "absolute",bottom: "10px",right: "10px",display: "flex",alignItems: "center",
            }}
          >
            <div
              className="hover-visible"
              style={{color: "skyblue",cursor: "pointer",fontSize: "20px",padding: "4px 7px",borderRadius: "5px",
              }}
            >
            ...
            <div
              className="hover-visible"
              style={{
                color: "skyblue", cursor: "pointer", fontSize: "20px", padding: "4px 7px", borderRadius: "5px",
              }}
            >
              <Button
            ...
            <div
              className="hover-visible"
              style={{
                color: "skyblue", cursor: "pointer", marginLeft: "10px", fontSize: "20px", border: "1px solid white", padding: "4px 7px", borderRadius: "5px",
              }}
            >
            ...
            <div
              className="hover-visible"
              style={{
                color: "skyblue", cursor: "pointer", marginLeft: "10px", fontSize: "20px", border: "1px solid white", borderRadius: "5px",
              }}
            >
              <ObjectDetection cameraId={camera.id} cameraIP={allcameradata.cameraIP} publicUrl={camera.rtspurl} cameraName={camera.name}
              />
            </div>
          </div>
        </>
      ...
      <Modal show={show} onHide={handleClose} size="lg">
        ...
        <Modal.Body>
          <iframe
            ...
            loading="lazy"
            allowFullScreen
            src={`https://www.openstreetmap.org/export/embed.html?bbox=${camera.longitude
              },${camera.latitude},${camera.longitude},${camera.latitude
