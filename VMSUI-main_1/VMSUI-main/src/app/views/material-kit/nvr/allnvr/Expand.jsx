...
function Expand({ data }) {
  ...
  const submitCameras = async () => {
    ...
    try {
      for (const camera of selectedCameras) {
        const response2 = await fetch(`${Addcamera}/camera_details`, {
          ...
          body: JSON.stringify({
            ...
            nvr: nvrData.id,
            area: nvrData.zone || "None",
            location: nvrData.location || "India",
      ...
      await delay(1000 * selectedCameras.length);
    } catch (err) {
  ...
  const fetchCameraDetails = async () => {
    ...
    try {
      const response = await fetch(`${API}/api/Camera/?user_id=${userId}`);
      ...
      setAddCameraData(data.results); // Assuming the response is an array or object with camera details
    } catch (err) {
