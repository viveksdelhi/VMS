...
export default function PaginationTable() {
  ...
  const handlePageSizeChange = async (event) => {
    try {
      if (event.target.value === "all") {
        ...
        //   `${API}/api/NVR/Count`, // Corrected endpoint syntax
        //   {
        //     headers: {
        ...
        //   }
        // );
        // const count = response.data.totalNVR;
  ...
  const getApi = async () => {
    try {
      const response = await axios.get(
        `${API}/api/NVR/?user_id=${userId}&page=${pageNumber}&page_size=${pageSize}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      setNvrList(response.data.results);
  ...
  const handleEditSubmit = async (id) => {
    ...
    const previousImages = selectedNvr.img.split(",");
    if (previousImages && previousImages.length > 0) {
      for (const img of previousImages) {
        const file = await convertURLToBlob(img); // Convert URL to Blob
        formData.append("img", file);
      }
    }
    formData.append("name", selectedNvr.name);
    formData.append("username", selectedNvr.username);
    formData.append("nvrip", selectedNvr.nvrip);
    formData.append("port", selectedNvr.port);
    formData.append("make", selectedNvr.make);
    formData.append("model", selectedNvr.model);
    formData.append("location", selectedNvr.location);
    formData.append("userid", userId);
    formData.append("zone", selectedNvr.zone);
    formData.append(
      "responsible_Person",
      selectedNvr.responsible_Person?.split(",")[0] +
        "," +
        selectedNvr.responsible_Person?.split(",")[1]
    );
    try {
      const response = await axios.put(`${API}/api/NVR/${id}/`, formData, {
        headers: {
    ...
    <Box sx={{ backgroundColor: "white" }} width="100%" overflow="auto">
      ...
      <Dialog open={openEditDialog} onClose={() => setOpenEditDialog(false)}>
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => handleEditSubmit(selectedNvr.id)}
            color="primary"
          >
            Save
          </Button>
