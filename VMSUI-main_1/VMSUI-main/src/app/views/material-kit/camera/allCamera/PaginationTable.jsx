...
import { API, detection, StreamAPI, token, userId, Stream } from "serverConnection";
...
export default function CameraList() {
  ...
  const handlePageSizeChange = async (event) => {
    try {
      if (event.target.value === "all") {
        ...
        //   `${API}/api/Camera/Count`, // Corrected endpoint syntax
        //   {
        //     headers: {
        ...
        //   }
        // );
        // const count = response.data.totalCamera;
  ...
  useEffect(() => {
    const fetchCameras = async () => {
      try {
        const response = await axios.get(
          `${API}/api/Camera/?user_id=${userId}&page=${pageNumber}&pageSize=${pageSize}/`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        if (status === "Active") {
          filteredData = filteredData.filter((camera) => camera.status === 1);
        } else if (status === "Inactive") {
          filteredData = filteredData.filter((camera) => camera.status === 0);
  ...
  const fetchCameras = async () => {
    try {
      const response = await axios.get(
        `${API}/api/Camera/?user_id${userId}&pageNumber=${pageNumber}&pageSize=${pageSize}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleConfirmDelete = async () => {
    ...
    if (currentCamera) {
      ...
      try {
        ...
        try {
          await axios.delete(`${Stream}/remove_camera/${currentCamera.id}`);
        } catch (err) {
          console.error("Failed to remove camera from Stream:", err.message);
        }
        // await axios.post(`${detection}/details`, { cameras: [postData] });
        // fetchCameras();

        await axios.post(`${StreamAPI}/stop_recording`, {
  ...
  const handleFormSubmit = async () => {
    if (currentCamera) {
      try {
        ...
        await axios.put(
          `${API}/api/Camera/${currentCamera.id}/`,
          updatedCamera,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const downloadAsExcel = () => {
    const formattedResults = searchResults.map((item, index) => ({
      ...
      Zone: item.area.charAt(0).toUpperCase() + item.area.slice(1),
      Model: item.brand,
  ...
  const downloadAsPDF = () => {
    ...
    const tableRows = searchResults.map((camera, index) => [
      ...
      camera.name,
      camera.cameraIP,
      camera.area.charAt(0).toUpperCase() + camera.area.slice(1),
      camera.brand,
      camera.macAddress,
      camera.manufacture,
      camera.location,
    ...
    <Box sx={{ backgroundColor: "white" }} width="100%" overflow="auto">
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button onClick={() => handleFormSubmit(formValues.id)}>Save</Button>
        </DialogActions>
      </Dialog>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <iframe
            ...
            loading="lazy"
            allowfullscreen
            src={`https://www.openstreetmap.org/export/embed.html?bbox=${coordinates.long},${coordinates.lat},${coordinates.long},${coordinates.lat}&layer=mapnik&marker=${coordinates.lat},${coordinates.long}`}
