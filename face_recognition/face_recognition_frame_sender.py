...
def process_video(camera_url, camera_id, user_id, credit_id, rabbitmq_host, queue_name, frame_interval, retry_limit=50):
    """
    ...
    while retry_count < retry_limit:
        ...
        if not channel or not channel.is_open:
            log_error(f"Error: Could not open RabbitMQ connection for {queue_name}")
            connection, channel = setup_rabbitmq_connection(queue_name, rabbitmq_host)
...
def start_camera_process(camera_url, camera_id, user_id, credit_id, rabbitmq_host, queue_name="frames_for_face_recognition", frame_interval=25):
    """
    Start a separate process for each camera.
    """
    process = Process(target=process_video, args=(camera_url, camera_id, user_id, credit_id, rabbitmq_host, queue_name, frame_interval))
    process.start()
...
def monitor_camera_processes(rabbitmq_host="rabbitmq", frame_interval=25):
    while True:
        for camera_id, process in list(camera_processes.items()):
            ...
            if not process.is_alive():
                ...
                if camera_id in camera_urls:
                    ...
                    start_camera_process(camera_url, camera_id, user_id, credit_id, rabbitmq_host, frame_interval)
                else:
...
def fetch_camera_data_from_queue(queue_name, rabbitmq_host="rabbitmq"):
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            running_status = camera_data.get('Running')
            ...
            if running_status == "TRUE":
                ...
                if camera_id not in camera_processes or not camera_processes[camera_id].is_alive():
                    ...
                    start_camera_process(camera_url, camera_id, user_id, credit_id, rabbitmq_host)
            else:
...
if __name__ == "__main__":
    ...
    fetch_camera_data_from_queue(queue_name="camera_details_for_face_recognition")
