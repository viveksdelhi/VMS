...
import { ANPRAPI, API, detection, token, userId } from "serverConnection";
import {
  Table,
  TableBody,
  TableCell,
...
function Anpr() {
    ...
    const fetchTableData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API}/api/NumberPlateReadedData/?user_id=${userId}&page=${currentPage}&pageSize=${itemsPerPage}&camera_id=${cameraId}`,
          {
            headers: {
  ...
  const handleItemsPerPageChange = async (event) => {
    try {
      if (event.target.value === "all") {
        // const response = await axios.get(
        //   `${API}/api/NumberPlateReadedData/Count`, // Corrected endpoint syntax
        //   {
        //     headers: {
        //       Authorization: `Bearer ${token}`,
        //     },
        //   }
        // );
        // const count = response.data.totalDetection;
  ...
  //   const filterResults = result.filter((alert) => {
  //     const alertDate = new Date(alert.regDate); // assuming `date_time` is in ISO string format
  //     // console.log({ aletr: alertDate });
  //     // console.log({ start: start });
  //     // console.log({ end: end });
  //     return alertDate >= start && alertDate <= end;
  //   });
  //   console.log(filterResults);
  //   // setSortedResults(filterResults);
  //   // console.log(object);
  // };
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "ANPR Data");
    XLSX.writeFile(wb, "anpr_data.xlsx");
  };
      ...
      <div className="d-flex justify-content-between align-items-center">
        ...
        <div className="d-flex">
          {/* <div>
            ...
            />
            <button onClick={handleDateFilter(sortedResults)}>Filter</button>
          </div> */}
          <Box>
                ...
                <TableRow key={data.id}>
                  ...
                  >
                    <IconButton
                      onClick={() =>
                        handleImageClick(
                          `${ANPRAPI}/${data.framePath.replace(/\\/g, "/")}`
                        )
                      }
                      color="primary"
                      size="small"
                  ...
                  >
                    <IconButton
                      onClick={() =>
                        handleImageClick(
                          `${ANPRAPI}/${data.platePath.replace(/\\/g, "/")}`
                        )
                      }
                      color="primary"
                      className="ms-2"
