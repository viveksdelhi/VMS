...
const CameraDetailsTable = () => {
  ...
  const handleToggle = async (row, field, value) => {
    ...
    setData(prev =>
      prev.map(cam => (cam.id === row.id ? { ...cam, [field]: value ? 1 : 0 } : cam))
    );
    ...
    try {
      ...
      if (field === 'isStreaming') {
        if (value) {
          ...
          // Stop streaming
          try {
            await axios.delete(`${STREAMING_API_URL}/Streaming/remove_camera/${row.id}`);
        ...
        if (value) {
          ...
          // Stop recording
          try {
            await axios.delete(`${RECORDING_API_URL}/Recording/stop/${row.id}`);
  ...
  const columns = [
    ...
    columnHelper.accessor('name', {
      header: 'Camera Name',
      cell: info => <span className="font-semibold text-[#2c028d]">{info.getValue()}</span>,
    }),
    ...
    columnHelper.accessor('status', {
      ...
      cell: info => {
        ...
        return <Tag color={val === 0 ? 'green' : 'red'}>{val === 0 ? 'Online' : 'Offline'}</Tag>;
      },
    }),
    columnHelper.accessor('isRecording', {
      ...
      cell: info => (
        <Switch
          ...
          onChange={val => handleToggle(info.row.original, 'isRecording', val)}
        />
      ),
    }),
    columnHelper.accessor('isStreaming', {
      ...
      cell: info => (
        <Switch
          checked={!!info.getValue()}
          onChange={val => handleToggle(info.row.original, 'isStreaming', val)}
        />
      ),
    }),
        ...
        return (
          <div className="flex gap-3 items-center">
            ...
            <EditOutlined
              ...
              onClick={() => navigate(`/camera/form`, { state: { camera: row } })}
            />
            <Popconfirm
      ...
      <div className="flex px-2 py-3 rounded-md flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-[#9864db] text-[#E6E6FA]">
        ...
        <div className="flex gap-3 items-center flex-wrap">
          ...
          >
            ...
            <option value="recording" className="text-black">
              Recording
            </option>
            <option value="streaming" className="text-black">
              Streaming
            </option>
            <option value="analytics" className="text-black">
              Analytics
            </option>
          </select>
          <select
      ...
      >
        {viewCamera && (
          <div className="space-y-2">
            <p>
              <b>ID:</b> {viewCamera.id}
            </p>
            <p>
              <b>Name:</b> {viewCamera.name}
            </p>
            <p>
              <b>IP:</b> {viewCamera.cameraIP}
            </p>
            <p>
              <b>Location:</b> {viewCamera.location}
            </p>
            <p>
              <b>Zone:</b> {viewCamera.zone}
            </p>
            <p>
              <b>Brand:</b> {viewCamera.brand}
            </p>
            <p>
              <b>Manufacture:</b> {viewCamera.manufacture}
            </p>
            <p>
              <b>MAC:</b> {viewCamera.macAddress}
            </p>
            <p>
              <b>Port:</b> {viewCamera.port}
            </p>
            <p>
              <b>Channel:</b> {viewCamera.channelId}
            </p>
            <p>
              <b>Latitude:</b> {viewCamera.latitude}
            </p>
            <p>
              <b>Longitude:</b> {viewCamera.longitude}
            </p>
            <p>
              <b>RTSP URL:</b> {viewCamera.rtspurl}
            </p>
            <p>
              <b>Status:</b> {viewCamera.status === 1 ? 'Online' : 'Offline'}
            </p>
            <p>
              <b>Recording:</b> {viewCamera.isRecording ? 'Yes' : 'No'}
            </p>
            <p>
              <b>Streaming:</b> {viewCamera.isStreaming ? 'Yes' : 'No'}
            </p>
            <p>
              <b>Registered:</b> {new Date(viewCamera.regDate).toLocaleString()}
            </p>
          </div>
        )}
      </Modal>
