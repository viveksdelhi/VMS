...
import { Addcamera, API, ONVIFAPI, StreamAPI, token } from "serverConnection"; // Ensure these are correctly imported or defined
...
const SimpleForm = () => {
  ...
  const submitCameras = async () => {
    ...
    try {
      ...
      formDatas.append("responsible_Person", formData.RPname + "," + formData.RPnumber);
      ...
      const cameraRequests = camerasToSubmit.map((camera) => {
        const cameraDetails = {
          camera_name: camera["Camera Name"],
          camera_ip: camera["IP Address"] || "NONE", // Ensure this field is correct
          nvr: id,
          area: camera["Area"] || "NONE",
          location: camera["Location"] || "India",
          public_url: camera["Stream URI"],
        ...
        <Grid container spacing={1}>
          ...
          <Grid item lg={6} md={6} sm={12} xs={12} sx={{ mt: 1 }}>
            <TextField
              type="text"
              name="RPnumber"
              label="Responsile Person Contact Number"
              onChange={(e) =>
