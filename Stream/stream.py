...
def call_api_periodically_for_camera(camera_id, credit_id=1, event_id=1):
    ...
    while live_camera_status.get(camera_id, False) and not (stop_event and stop_event.is_set()):
        try:
            ...
            #response = requests.post("http://14.195.152.244:7007/transaction_update", json=data)
            stream_api= os.getenv("STREAM_API")
            stream_api= f"{stream_api}/transaction_update"
            response = requests.post(stream_api, json=data)
